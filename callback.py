    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.check_freq == 0:
            try:
                ...
                # If monitor files are not found yet, just continue
                if self.verbose >= 2:
...
class EarlyStopping_by_avg:
    """
    Utility class for early stopping based on average performance.
    This is not a callback but a utility class for manual early stopping.
    ...
    def check(self, avg, avg_scores):
        ...
        Args:
            avg (float): Current average score
            avg_scores (list): List of previous average scores
